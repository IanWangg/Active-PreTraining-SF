...
class Critic(nn.Module):
    def __init__(self, args):
        ...
        self.fc_v = nn.Sequential(
            nn.Linear(args.embed_dim, args.critic_hs), 
            nn.ReLU(), 
            nn.Linear(args.critic_hs, 1)
        )
        self.fc_a = nn.Sequential(
            nn.Linear(args.embed_dim, args.critic_hs), 
            nn.ReLU(), 
            nn.Linear(args.critic_hs, args.action_num)
        )
...
# fully connected universal successor feature approximator
class USFA_FC_Fourrooms(nn.Module):
    def __init__(self, args):
        super().__init__()
        # don't need args for now, keep args as input for consistency
        del args
        ...
        self.fc_a = nn.Sequential(
            ...
            nn.Linear(512, 1024),
            nn.ReLU(),
            nn.Linear(1024, 169 * 4)
        )

    def forward(self, state, w):
        batch_size = state.shape[0]
        a = self.fc_a(state).view(state.size(0), -1)  # (256, 169 * 4)
        v = self.fc_v(state).view(state.size(0), -1)  # (256, 169)
        a = a.view(batch_size, 4, 169)
        q = v + a - a.mean(1, keepdim=True).squeeze(1)  # (b, a)
        return torch.matmul(q, w).squeeze(-1)

class Phi_Fourrooms(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, state):
        return F.normalize(self.fc(state), p=2)
