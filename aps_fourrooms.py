...
# for fourrooms aps
import model.USFA_FC_Fourrooms as Critic
import model.DummyEncoder as Encoder
...
'''
Difference between APT and APS: (For APS)
1. when calculating the exploration reward, use Phi(s') instead of s'
2. use [- Phi(s) * w] as loss for training Phi, don't use contrastive loss
3. need to l2-normalize w and Phi  ====  (done)
'''

'''
Note that the eval() function after a module is just to turn off 
the functionality that we don't want during evaluation
'''
...
class Algo(object):
    ...
    @torch.no_grad()
    def select_action(self, state, w, stochastic, epsilon=0.001):
        ...
        if stochastic and np.random.rand() < epsilon:
            ...
            current_Q = self.critic(self.state_encoder(state), w)
    ...
    def train(self, buffer, w, args):
        ...
        with torch.no_grad():
            next_action = self.critic(self.state_encoder(next_state), w).argmax(-1).view(-1, 1).long()  # (b, a) -> (b, 1)
            target_Q = self.critic_target(self.state_encoder_target(next_state), w).gather(-1, next_action)  # (b, 1)
    ...
    def train_repr(self, state, w, args):
        repr_loss = torch.matmul(self.state_encoder(state), w).mean()
